...
export default function AdvancedSearch ({articles, search, country}){
    ...
    return(
        ...
        <div className='m-auto advanced-contain '>
            <AdvancedSearchForm/>
        </div>
            
        
        <h4 className='ml-4 mt-4 text-my-light-blue oswald'>Search Results {search ? `for "${search}"`:''} {
...
export const getServerSideProps= async ({query})=>{
    const res= await Axios.get(`https://gnews.io/api/v3/search?q=${query.search}&country=${query.country}&lang=en&token=${process.env.NEWS_API_KEY}`);
    return {
        props:{
